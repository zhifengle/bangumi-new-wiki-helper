...
describe('test getchu function', function () {
  it('deal title text', function () {
    ...
    expect(dealTitle(`はつゆきさくら 通常版

 （このタイトルの関連商品）`))
      .toEqual('はつゆきさくら')
    expect(dealTitle('神様のような君へ 初回版＜早期予約キャンペーン特典＞ '))
      .toEqual('神様のような君へ')
    expect(dealTitle('VenusBlood -HYPNO- Win10対応廉価版'))
      .toEqual('VenusBlood -HYPNO-')
    expect(dealTitle('Summer Pockets REFLECTION BLUE 豪華限定版＜早期予約キャンペーン特典付き＞ '))
      .toEqual('Summer Pockets REFLECTION BLUE')
    expect(dealTitle('フルキスS 初回限定生産版 '))
      .toEqual('フルキスS')
    expect(dealTitle('恋する乙女と守護の楯 Re:boot The“SHIELD-9” 完全生産限定版＜早期予約キャンペーン特典付き＞ '))
