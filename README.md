...
现实人物支持 VGMdb 的艺术家（artist）与组织（org）页面。
...
- 检测条目是否在 bangumi 上存在
- 从 VGMdb 艺术家 / 组织页面提取信息，填写新建现实人物的表单并上传肖像；「新建人物并查重」会先搜索 bangumi 上是否已有同名人物
- 新建角色时，按钮“添加人物并上次肖像”旁边输入关联条目 ID，可以自动关联条目。如果抓取的信息里面有 CV 。会搜索和关联 CV
- 处理封面，支持图片打码
- 「导出 JSON」把提取到的条目 / 角色 / 人物信息导出为 JSON 文件或复制到剪贴板，方便交给其它工具处理
...
`build/rollup.config.ext.mjs` 查看和配置浏览器扩展的对应入口
